...
// This is copied from the tests in virtualization-lms-core. It is used to support recursion.
...
trait FunctionsExp extends Functions with BaseExp { // shadow trait with same name in core package
  ...
  def doLambda2[A1:Manifest,A2:Manifest,B:Manifest](fun: (Exp[A1],Exp[A2]) => Exp[B]) : Exp[(A1,A2) => B] = Lambda2(fun)
...
trait FunctionsExpClever extends FunctionsExp {
...
trait JSFunctionsExp extends JSFunctions with FunctionsExp with FunctionExpUnfoldRecursion with FunctionsExternalDef2 with FunctionsExternalDef0 with EffectExp

trait JSGenFunctions extends JSGenEffect {
  val IR: JSFunctionsExp //FunctionsExternalDef0 with EffectExp
  import IR._

  override def emitNode(sym: Sym[Any], rhs: Def[Any])(implicit stream: PrintWriter) = rhs match {
    case e@DefineFun(y) =>
      stream.println("var " + quote(sym) + " = function(" + quote(e.arg) + ") {")
      emitBlock(y)
