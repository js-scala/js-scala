...
This implementation may be more readable, however the code is wrong: if we call \code{hello("<me>")} the brackets won’t be escaped and will produce a \code{ME} tag, which is not the expected behavior. So, although this way reads better it’s not really less error prone.
...
Our solution has both the safety and performance of the listing \ref{efficient-null} and the expressiveness of the listing \ref{null-unsafe}. We encode the potential emptiness of a value of type \code{Rep[A]} using the \code{Rep[Option[A]]} type at the stage level but we don’t generate code wrapping values in a container, instead we just check if a value is \code{null} or not. Here is a listing using our DSL:
...
To workaround these issues, developers can wrap the variable declaration and the \code{inc} function code in a factory function (cf listing~\ref{state-encapsulated}). Thus the state would be hidden and developers could not forget the initialization.
...
Instead, we propose a monadic DSL that does not attempt to hide its asynchronous nature but provides a way to lift asynchronous values as first-class citizen and, thanks to the \code{for} notation, a convenient syntax to avoid the callback-hell.
...
Because callbacks are explicitly passed to the \code{foreach}, \code{map} and \code{flatMap} methods, there is no risk of ambiguity such as the one of listing \ref{async-ambiguous}. The Scala \code{for} notation handles the callback-hell problem.
...
% TODO mentionner le traitement de cas d’erreurs ?
