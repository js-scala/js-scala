...
This paper continues their work and presents several safe and expressive DSL constructs for RIAs development. We
increase the expressivity of JavaScript native APIs with no runtime overhead and expose them through statically typed
APIs. We make asynchronous programming simpler by raising asynchronous computations to first-class citizens. We
leverage the Scala type system to bring type-directed language features such as \emph{ad hoc} polymorphism and type
coercion to the client-side.
...
write the expression \code{xs.size == 0}, how should it be translated to the client-side environment
...
type of the event passed to the handler varies with the event name. For instance, in listing \ref{event-js} the
...
distinction between synchronous and asynchronous parts.
...
We limited the DSL to two operations (\code{foreach} and \code{isEmpty}) for the sake of brevity. The
...
programming language constructs both safer and more expressive without abstraction penalty. We focused on the main
...
each asynchronous computation, a \code{Promise} value registering the callbacks to call when the value is available.
...
so before each further method call it tests the emptiness of the container and applies effectively the operation only
