...
Asynchronous values can be used and transformed using the \code{foreach}, \code{map} and \code{flatMap} methods, turning them into first-class
...
Asynchronous values are reified in values with a \code{put} and a \code{get} operations. The \code{put} operation
sets the value of the asynchronous value and the \code{get} operation uses the asynchronous value when available.
All other operations (\code{foreach}, \code{map} and \code{flatMap}) are implemented in terms of these two primitives.
...
renamed some identifiers for the sake of readability). The \code{x1} and \code{x2} values reify the asynchronous
results of the Ajax requests, they contain a field \code{v} holding the result value (initialized to \code{null}) and an
array of callbacks \code{cs} (initially empty) to call when the result is available. The \code{AjaxGet} function code
has been omitted, it creates a \code{XMLHttpRequest} object that sends a HTTP request and completes the value of its
last parameter with the response, when received, using the \code{put} operation. The implementation of \code{put} sets
the \code{v} field to the value and calls all the pending \code{cs} callbacks. The \code{Get} function shows the
implementation of the \code{get} primitive. It takes two parameters, an asynchronous value and a callback. If the value
...
compilation step desugars the \code{for} notation to calls to \code{foreach}, \code{map} and \code{flatMap}. These
methods are implemented in terms of the \code{put} and \code{get} primitives, so they evaluate to an intermediate
representation containing \code{Put} and \code{Get} nodes. Finally, these nodes are translated into the code actually
