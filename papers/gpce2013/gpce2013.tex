...
and stop further JavaScript execution. Defensive code can handle \code{null} reference, but leads to very cumbersome
...
\code{for} notation is used to sequence computations over optional values encoded with a monad, allowing a high
level of expressiveness. (To be continued, mention \code{Rep[Option[A]]}, \code{Rep[Some[A]]}, \code{Rep[None.type]})
