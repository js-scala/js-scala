...
The trait contains just one method, \code{log}, representing the action of appending a message to the log.
...
inherits from \code{BaseExp} which aliases \code{Rep} to
\code{Exp}. \code{Exp} represents an IR via two subclasses:
\code{Const} for constants and \code{Sym} for symbols referring to a composite expression. Composite expressions are represented by type
\code{Def}.

In our
\code{DebugExp} trait, we extend \code{Def} to define a new IR node:
\code{Log}. This node represents a statement appending a message to the log.

An LMS program returns a value of type \code{Rep[_]}, a graph of IR nodes, representing the staged program. This graph is traversed and a code generator is invoked to transform each node into the corresponding code in the target language. To
...
We chose to implement the logging action by using the standard \code{console.log} Web browser's function.
